...
- `/kn-flow @doc/<spec-path>` - The normal gated pipeline; prefer it over go mode
- `/kn-plan --from @doc/<spec-path>` - Generate tasks only, keeping approval gates
- `/kn-commit` - The single mandatory commit gate in go mode

## Red Flags

- Invoking go mode without an explicit user request for legacy behavior
- Skipping the commit gate because other gates are disabled
- Auto-accepting Decision candidates while running unattended phases
