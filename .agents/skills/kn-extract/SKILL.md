...
- `/kn-doc` - Author the full canonical doc a memory or learning points to
- `/kn-implement` - Where the System Decision Impact checkpoint originates during task completion
- `/kn-template` - Promote a repeated generatable pattern into a template (requires linked pattern doc)

## Red Flags

- Creating Memory category `decision` instead of a draft System Decision candidate
- Copying Spec Locked Decisions into the System Decision ledger
- Auto-accepting or marking a Decision candidate current
- Promoting marginal learnings into learnings/critical-patterns
- Extracting generic advice with no source provenance
